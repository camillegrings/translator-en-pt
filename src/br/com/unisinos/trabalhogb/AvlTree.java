...
public class AvlTree {
	...
	private int getFator (AvlNode node) {
	...
	private AvlNode insert (Dicionario d, AvlNode node) {
		...
		*/
		...
		if ( getFator(node) == 2 ) {//Se o fator for 2, a árvore não está balanceada
			if (getFator (node.getLeft())>0)    
		...
		else if ( getFator(node) == -2 ) {//Se o fator for -2, a árvore não está balanceada
			if ( getFator(node.getRight())<0 ) 
		...
		/*
		     ...
		     |1| |3| (0+1)     
		 */
		node.setHeight( getMaior( height( node.getLeft() ), height( node.getRight() ) ) + 1 );  
		return node;
	}
	...
	private static AvlNode rotateSimpleToRight( AvlNode k2 ) {
		...
		k2.setHeight( getMaior( height( k2.getLeft() ), height( k2.getRight() ) ) + 1 ); // A altura de k2 é definida pela maior altura entre suas sub arvores direita e esquerda, + 1
		k1.setHeight( getMaior( height( k2.getLeft() ), k2.getHeight() ) + 1 );          //A altura de k1 é determinada pela maior altura entre a altura de k2 e a altura da sub arvore esquerda de k1
	...
	private static AvlNode rotateSimpleToLeft( AvlNode k1 ) {
		...
		k1.setHeight( getMaior( height( k1.getLeft()), height( k1.getRight() ) ) + 1 );
		k2.setHeight( getMaior( height( k2.getRight() ), k1.getHeight() ) + 1 );
	...
	protected AvlNode buscaPalavra(AvlNode node, String palavra) {
		/*
		 * Percorre a árvore para a direita ou esquerda (depende da palavra pesquisada).
		...
		if ( node == null ) {
			...
			else if ( palavra.compareTo( node.getKey().getPalavra() ) < 0 ) {
